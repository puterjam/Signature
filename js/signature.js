...
(function Signature() {
     ...
     * @type {Array}
     */
    var ITEM_HTML = ['<div style="text-align: center">', '	<p>', '		<img id="%sign_img%" style="max-width:100%" class="img-polaroid"/>', '		<canvas id="%sign_canvas%" width="%width%" height="%height%" style="display:none"></canvas>', '	</p>', '	<p>', '		<a class="btn btn-small btn-success" data-action="save" data-id="%sign_canvas%" download="%id%.png" target="_blank">保存 [%name%]</a>', '		<a class="btn btn-small btn-copy" data-action="copy" data-id="%sign_img%" data-clipboard-target="#%sign_img%" target="_blank">复制 HTML</a>', '	</p>', '</div>'].join("");
    /**
     * 字体列表
     * @type {Array}
     */
    var FONTS = ["default", "-", "Arial", "Arial Black", "微软雅黑", "宋体", "-", "kaiti SC", "Songti SC", "Hiragino Sans GB W3", "Helvetica Neue", "-", "ABeeZee", "Carrois Gothic SC", "Numans", "Ubuntu"];
    /**
     * 字体合法的规则
     * @type {Array}
     */
    ...
    var drawPage = function() {
            ...
            for (var k in currentThemes) {
                var _item = ITEM_HTML.replace(/%id%/g, k).replace(/%sign_img%/g, k + "_img").replace(/%sign_canvas%/g, k + "_canvas").replace(/%name%/g, currentThemes[k].name).replace(/%width%/g, currentThemes[k].width).replace(/%height%/g, currentThemes[k].height);
                html.push(_item);
            }
    ...
    var saveUserInfo = function() {
        _val["rtx"] = $("#rtx_input").val();
        _val["group"] = $("#group_input").val();
        _val["phone"] = $("#phone_input").val();
        _val["mobile"] = $("#mobile_input").val();
        _val["address"] = $("#address_input").val();
        localStorage.setItem("userInfo", JSON.stringify(_val));
    }
    var loadUserInfo = function() {
            _val = JSON.parse(localStorage.getItem("userInfo")) || {};
            $("#rtx_input").val(_val["rtx"] || "");
            $("#group_input").val(_val["group"] || "");
            $("#phone_input").val(_val["phone"] || "");
            $("#mobile_input").val(_val["mobile"] || "");
            $("#address_input").val(_val["address"] || "");
        }
        /** 渲染所有的签名档 */
    ...
    var _drawSign = function drawSign(ctx, theme) {
            ...
            //绘制名称
            theme.textStyle.name && _fillText(ctx, _val["rtx"], theme, "name");
            //绘制团队名称
            //var w = ctx.measureText(name).width;
            theme.textStyle.group && _fillText(ctx, _val["group"], theme, "group");
            //绘制电话号码
            theme.textStyle.phone && _fillText(ctx, _val["phone"], theme, "phone");
            //绘制手机号码
            theme.textStyle.mobile && _fillText(ctx, _val["mobile"], theme, "mobile");
            theme.textStyle.address && _fillText(ctx, _val["address"], theme, "address");
        }
        /**
         * 填充文字
    ...
    var initFontList = function() {
            ...
            FONTS && FONTS.forEach(function(value) {
                if (value == "-") {
                    ...
                    var _start = '<li class="' + (value == "default" ? 'active' : '') + '"><a href="#' + value + '" font-family="' + value + '" style="font-family:' + (value == "default" ? '' : value) + ';">' + (value == "default" ? '使用主题推荐' : value) + '</a>';
                    //TODO 暂时保留以后增加字体样式
         ...
         * @param {dom} dom对象
         */
    var _doSave = function(target) {
            var canvasId = target.dataset["id"];
            //判断对象是否是按钮
            if (canvasId) {
         ...
         * @param {dom} dom对象
         */
    var _doCopy = function(target) {
        var imageId = target.dataset["id"];

        if (imageId && !target.classList["disabled"]) {
        	target.classList.add("disabled");
            var img = document.getElementById(imageId);
            var div = document.createElement("div");
            div.contentEditable = true;
            div.innerHTML = '<br/><img src="' + img.src + '" style="width:' + img.offsetWidth + 'px; height:' + img.offsetHeight + 'px"/>';
